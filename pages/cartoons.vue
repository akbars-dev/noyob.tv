...
const fetchMovies = async () => {
    const response = await useFetch(`https://back.noyob.tv/api/v1/cartoons?limit=${limit}&offset=${offset.value}`);

    if (response.data.value) {
        movies.value = response.data.value.cartoons;
